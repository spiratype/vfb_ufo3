...
ignored. A batch command can be created for use with psautohint, which supports
hinting outlines with decimal coordinates.

REQUIREMENTS
This package has no dependencies outside of the standard library. It is written
in Cython and in Python 3 syntax where it is natively supported by Cython. The
submodules are compiled into .pyd extension modules. To recompile the
submodules, the PyPi cython package and a compiler for Cython to utilize during
extension module compilation will be required.
...
instance_values are not provided for a multiple master source font, a UFO will
be generated for each master in the font.

Fonts with a large number of glyphs benefit greatly from supplying additional
glyph names to be optimized (glyphs_optimize_names) and/or glyph names and
suffixes which can be omitted from the final UFO instance (glyphs_omit_names,
glyphs_omit_suffixes). See GLYPHS OPTIONS below.

Generated instances/layers can be saved and/or left open after generation via
the vfb_save and vfb_close options. If vfb_save is set to True, the resulting
.vfb instance will be updated during UFO creation. This includes glyph outline
changes (overlap removal and decomposition).
...
configurable; please see CONFIGURABLE_ATTRS in the fontinfo.pxi source file for
a list of attributes which will be checked and updated to the UFO(s) during
creation.
...
scale_to_upm option is ignored if it is lower than 1000. Scaling can be turned
off by setting scale_auto to False; this does not reduce conversion times.

All scaling operations are performed independently from FontLab; if the .vfb
instance(s)/master copy are being saved, the .vfb font and glyph values will be
the original un-scaled values.
...
or list-form with a value for each axis. For single-axis fonts, if the list
values are numerical, the values will be converted to single-element lists:
...
The generated .vfbinstance(s) will leave components in component-form.

To disable the optimizations outlined above, set the glyphs_optimize option to
False.
...
Optimize glyph name and code point lists
  ...
  of '0x00ac'-format strings, integers in hexadecimal-form (0x00ac), or numeric
  integers (172); values which cannot be converted to an integer will raise a
  ValueError. Only the first code point in each glyph's list of code points
  (FontLab unicodes attribute) is checked for code point set membership.
...
features_import_groups to True. The font's features are neither formatted nor
checked for correctness. Users are responsible for moving referenced feature
files from include() statements to the chosen output directory. Also see KERN
FEATURE OPTIONS and MARK FEATURE OPTIONS below for kern and mark feature
options.
...
kern_min_value. This value should be an integer and when set, all kern values
(negative and positive) not above the threshold will be omitted from the kern
feature.
...
kern_feature_generate to False will turn this off. The kern feature generation
will add subtables and a lookup as necessary. This is not particularly elegant,
and so far, no checks are made to guarantee a working kern feature. Any
remaining subtable overflows may be due to glyph(s) being in more than one kern
group of the same side; however overflows can also be caused by issues from one
or more GPOS features located earlier in the feature list.

Mark feature options
A mark feature can be generated on export by setting mark_feature_generate to
True. A list of anchor names to omit (mark_anchors_omit) or a list of anchor
names to include (mark_anchors_include) can be supplied to fine-tune the mark
feature output.

Group options
Providing a FontLab-class file (.flc) or groups.plist speeds up UFO creation
time significantly when the group names are not named using first and second
group identifiers (see groups_flc_path and groups_plist_path options). Group
names in the .flc file do not have match any specific formatting (e.g.
MMK_R_<key glyph>, public.kern2.<key glyph>).

When not using either a .flc or groups.plist file, group names will be checked
for UFO3-style group identifiers (public.kern1.<key glyph>, public.kern2.<key
glyph>), MetricsMachine-style identifiers (MMK_L_<key glyph>, MMK_R_<key
glyph>), and the simpler _L and _R identifier suffixes. Groups which either
have no kerning or not identifiable using their name will be identified as
first/second using FontLab's built-in GetClassLeft/GetClassRight methods.

If the export_flc option is set to True, a FontLab-class file (.flc) will be
generated with group names matching those of the generated UFOs. This file will
be located in the same directory as the generated UFOs.
...
AFDKO options
The OS/2, hhea, head, and name tables will be added to the features file. The
name table entry strings will be formatted according to OpenType Feature File
Specification § 9.e. Any strings unable to be formatted fully for each
platform's specific encoding restriction (Windows -- UTF-8, Macintosh -- Mac
Roman) will be formatted to their nearest ASCII equivalent rather than omitting
any un-encodable characters. The standard library unicodedata module is used
for entries that meet this need.
...
Optionally, the first 256 glyphs can be filled from the Windows-1252 or Mac OS
Roman code pages (afdko_makeotf_GOADB_win1252,
afdko_makeotf_GOADB_macos_roman). The first character of a generated GOADB file
will always start with the .notdef glyph. If a GOADB file is provided, it is
not checked for correctness. Commands to run MakeOTF for each generated
...
instances as a batch using the options psautohint_cmd and psautohint_batch_cmd,
respectively. The default options are -d (write decimal (float) hint
coordinates) and -w (write hints directly to the .glif lib for each glyph).
...
A .designspace document can be created in place of individual UFO instances. A
UFO for each master will be generated and the instances will be described in
the .designspace document. A default instance can be described with the
designspace_default option. This value must be a list or tuple with a value for
each axis in the font. If glyphs_omit_list or glyphs_omit_suffixes_list lists
are provided, the glyphs will remain in the source UFOs and a glyph mute rule
for each glyph to be omitted will be added for each instance.

Benchmarks
For reference, testing was performed on a Windows 10 machine with an Intel Xeon
E5 1650v3 @ 3.5 GHz CPU and a solid-state hard drive.
...
UFO format. This prep time increases when not providing a FontLab-class or
groups.plist file.
