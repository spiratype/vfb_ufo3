...
def _parts(ufo):
  ...
  elif ufo.opts.afdko_makeotf_run or ufo.opts.afdko_makeotf_batch_run:
    batch = ufo.opts.afdko_makeotf_batch_run and len(ufo.instance_values) > 1
    makeotf_command(ufo, batch=batch, write=0)
...
def makeotf_command(ufo, batch=0, write=1):
  ...
  * -o  output path                                        >> default (<ufoname>.otf)
  ...
  if batch:
    ufo.afdko.makeotf.batch_cmd.append(command)
    if ufo.last and write:
      write_bat(ufo.afdko.makeotf.cmd, ufo.paths.afdko.makeotf_cmd)
  elif write:
    write_bat(command, ufo.paths.instance.makeotf_cmd)
  else:
    ufo.afdko.makeotf.cmd = command


def psautohint_command(ufo, batch=0, write=0):
  ...
  if batch:
    ufo.psautohint.batch_cmd.append(command)
    if ufo.last and write:
      write_bat(ufo.psautohint.cmd, ufo.paths.psautohint_cmd)
  elif write:
    write_bat(command, ufo.paths.instance.psautohint_cmd)
  else:
    ufo.psautohint.cmd = command

def write_bat(command, command_path):
