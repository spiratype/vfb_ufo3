...
DEFAULT_OPTIONS = (
  ...
  ('glyphs_decompose_names', []),
  ('glyphs_remove_overlap_names', []),
  ('glyphs_omit_names', []),
  ('glyphs_omit_suffixes', ()),
  ('glyphs_optimize', True),
  ('glyphs_optimize_makeotf', True),
...
def check_option(value_type, key, value):
  ...
  if value_type == 'str':
    ...
    if key in PATH_OPTIONS and not os_path_isabs(value):
      print(b"Skipping '%s' (given file path is not absolute)" % value.encode('cp1252'))
      return None
    if key in FILE_OPTIONS and not os_path_isfile(value) and key != 'groups_export_flc_path':
      print(b"Skipping '%s' (given file path is not present on file system)" % value.encode('cp1252'))
      return None
  ...
  if value_type == 'int':
    ...
    if not isinstance(value, int):
      raise ValueError(b"'%s' must be an integer." % key.encode('cp1252'))
    return value
