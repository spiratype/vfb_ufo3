...
def write_ufo(options):
  ...
  for instance in ufo.instances:
    ...
    if ufo.afdko or ufo.psautohint:
      fdk(ufo)
...
def parse_options(options):
  ...
  if opts.kern_feature_file_path and opts.kern_feature_file_path.endswith('.fea'):
    check_user_file(opts.kern_feature_file_path, 'kern')
    ufo.paths.kern_feature = opts.kern_feature_file_path

  if opts.groups_flc_path and opts.groups_flc_path.endswith('.flc'):
  ...
  if opts.groups_plist_path and os_path_basename(opts.groups_plist_path).endswith('groups.plist'):
    ufo.paths.groups_plist = opts.groups_plist_path

  if opts.glyphs_decompose_names:
    opts.glyphs_decompose_names = encode_string_list(opts.glyphs_decompose_names)

  if opts.glyphs_remove_overlap_names:
    opts.glyphs_remove_overlap_names = encode_string_list(opts.glyphs_remove_overlap_names)

  if opts.afdko_parts:
    opts.features_import_groups = 1
    if opts.designspace_export:
  ...
  if opts.glyphs_optimize or opts.glyphs_optimize_makeotf:
...
def unique_string_list(user_list):
  if isinstance(user_list, set):
    return [item for item in user_list if isinstance(item, basestring)]
  return [item for item in set(user_list) if isinstance(item, basestring)]
...
def encode_string_list(string_list):
  return {encoded(item) for item in unique_string_list(string_list)}

def decode_string_list(string_list):
  return {decoded(item) for item in unique_string_list(string_list)}
...
def copy_master_info(ufo):
  ...
  def font_repr(font):
    ...
    if font.axis:
      return f'<Font: {font.full_name!r} filename={filename!r} axes={len(font.axis)}>'
    return f'<Font: {font.full_name!r} filename={filename!r}>'
